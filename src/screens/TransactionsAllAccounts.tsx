...
const TransactionsScreen = ({onPressBack}) => {
  ...
  const filterTransactions = () => {
    ...
    if (amountFilter) {
      filtered = filtered.filter(transaction => Number(transaction.amount) === Number(amountFilter));
    }
  ...
  */}
  const CategoryFilterModal = () => (
    <Modal
      ...
      <Text style={{fontSize: 20, color: 'black', fontWeight: 'bold', marginBottom: 15}}> Filter By Category</Text>
        ...
        <Button title="Close" onPress={() => {setCategoryModalVisible(false), setShowMenu(false)}} />
      </View>
    </Modal>
  );
  ...
  const renderTransaction = ({item}) => (
    <View style={styles.row}>
      ...
      <Text style={[styles.transactionText, {fontWeight: 'bold', textAlign: 'center' ,color: 'white',/*backgroundColor: '#2196F3'*/ backgroundColor: "#6199F7", padding: 2,paddingVertical:4, fontSize: 12}]}>{item.modeOfPayment}</Text>
      </View>
      
      <Text style={[styles.cell,, {color: 'black'}]}>{item.category}</Text>
      { item.transactionType==='income' ? (
        <><Text style={[styles.cell, styles.transactionText]}>{item.amount}</Text> 
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Transactions - All Accounts</Text>
      ...
      <View style={styles.headerRow}>
        <View style={styles.cell}><Text style={styles.headerTable}>Date</Text></View>
        <View style={styles.cell}><Text style={styles.headerTable}>Category</Text></View>
        <View style={styles.cell}><Text style={styles.headerTable}>Income</Text></View>
        <View style={styles.cell}><Text style={styles.headerTable}>Expense</Text></View>
      </View>
    ...
    <View style={styles.footerBarcontainer}>
        ...
        <View style={{justifyContent: 'center', alignItems: 'center'}}>
          <Text style= {{color: 'white', fontWeight: 'bold', fontSize: 19 }}>{transactionsTotals.totalIncome}</Text>
          <Text style= {{color: 'white', }}>Total Income</Text>
        </View>

        <View style={{justifyContent: 'center', alignItems: 'center'}}>
          <Text style= {{color: 'white', fontWeight: 'bold', fontSize: 19}}>{transactionsTotals.totalExpense}</Text>
          <Text style= {{color: 'white', }}>Total Expense</Text>
        </View>

        <View style={{justifyContent: 'center', alignItems: 'center'}}>
          <Text style= {{color: 'white', fontWeight: 'bold',fontSize: 19 }}>{transactionsTotals.totalBalance}</Text>
      ...
      {showMenu && (
        ...
        <View style={styles.menu}>
          <TouchableOpacity  style={styles.menuOption} onPress={() => {setShowDatePicker(true), setShowMenu(false)}}>
          ...
          <TouchableOpacity style={styles.menuOption} onPress={() => {setAmountModalVisible(true) , setShowMenu(false)}}>
            <Text style={{color: 'black'}}>Amount</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.menuOption} onPress={() => {setSortingOption('DateAscending'), setShowMenu(false)} }>
            <Text style={{color: 'black'}}>Date Ascending</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.menuOption} onPress={() => {setSortingOption('DateDescending'), setShowMenu(false)} } >
            <Text style={{color: 'black'}}>Date Descending</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.menuOption} onPress={() => {setSortingOption('CategoryAscending'), setShowMenu(false)} } >
            <Text style={{color: 'black'}}>Category Ascending</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.menuOption} onPress={() => {setSortingOption('CategoryDescending'), setShowMenu(false)} } >
            <Text style={{color: 'black'}}>Category Descending</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.menuOption} onPress={() => {setCategoryModalVisible(true) , setShowMenu(false)}}>
            <Text style={{color: 'black'}}>Category</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.menuOption} onPress={() => console.log('Ordering')}>
      ...
      animationType="fade">
      <View style={styles.modalContainerAmount}>
        <Text style={{fontSize: 20, color: 'black', fontWeight: 'bold', marginBottom: 15}}> Filter By Amount</Text>
        <TextInput
          placeholder="Enter amount"
          value={amountFilter}
          onChangeText={setAmountFilter}
        ...
        <View style={{flexDirection: 'row', gap: 5, justifyContent: 'space-evenly', alignItems: 'streach', marginTop: 5}}>
        <TouchableOpacity  style={{backgroundColor: "#2196F3", flex: 1}} onPress={() => {
          setAmountModalVisible(false);
          filterTransactions();
        }}>
          <Text style={{fontWeight: 'bold', color: 'white', padding: 5, textAlign: 'center'}}>
            APPLY
          </Text>
        </TouchableOpacity>
        <TouchableOpacity style={{backgroundColor: "#2196F3", flex: 1}} onPress={() => {setAmountModalVisible(false), setShowMenu(false)}}>
          <Text style={{fontWeight: 'bold', color: 'white', padding: 5, textAlign:  'center'}}>CLOSE</Text>
        </TouchableOpacity>
        </View>
